...
func Build(app *models.App, outputContainer *fyne.Container) func() {
	return func() {
		if err := utils.Shellout(BuildAppDesc, fmt.Sprintf("cd %s && yarn build", app.LocalPath), outputContainer, true); err != nil {
...
func BuildArchive(app *models.App, outputContainer *fyne.Container) func() {
	return func() {
		...
		if err := utils.Shellout(CreateArchiveDesc, fmt.Sprintf("cd %s && tar cvf dist.tar %s", app.LocalPath, app.LocalDistDirName), outputContainer, false); err != nil {
			utils.AddTextToOutput(err.Error(), errorColor, outputContainer)
			return
		}
	}
